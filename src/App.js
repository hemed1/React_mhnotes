...
export default function App( {dbData, dbIndex} )      /* initialData */
{
  ...
  function showDataGridNotes()
  {
    ...
    const arrayColumns = [
                          ...
                          {caption: 'סוג פריט', fieldName: 'ListTypeID', type: 'number', width: '90px', color: '#303033'},
                          {caption: 'סטטוס', fieldName: 'StatusID', type: 'number', width: '90px', color: '#303033'},
                          {caption: 'תגיות', fieldName: 'SubjectLabels', type: 'string', width: '230px', color: '#303033'},
                          {caption: 'ת. ביצוע', fieldName: 'DateDue', type: 'date', width: '200px', color: '#303033'},
                          {caption: 'במועדפים', fieldName: 'IsFavorite', type: 'number', width: '70px', color: '#303033'},
                          {caption: 'מזהה רשומה', fieldName: 'FirebaseID', type: 'string', width: '280px', color: '#303033'},
                          {caption: 'ת. עדכון אחרון', fieldName: 'LastUpdateDate', type: 'date', width: '280px', color: '#303033'},
                        ]
  ...
  async function showDataGridChilds()
  {
    ...
    setGridData(
      <GridHandle.GridWidget data={dataChilds} title='ניהול נתונים' tableName='TBL_NotesChilds' arrayColumns={arrayColumns} 
                                        top='190px' left='150px' width='950px' height='900px' onSaveFuncName={onGridSaveFuncName} />
    );
  }
  ...
  async function showDataGridMainDatabase()
  {
    setIsShowGrid(!isShowGrid);
    setSelectedCode(2)
    ...
    setGridData(
      <GridHandle.GridWidget  data={dataBaseTable} title='ניהול מסד-נתונים ראשי' tableName='TBL_Databases' arrayColumns={arrayColumns} 
                              top='210px' left='150px' width='1730px' height='870px' onSaveFuncName={onGridSaveFuncName} />
    );
  ...
  async function showDataGridStatuses()
  {
    if (!isShowGrid)
    {
      ...
      const arrayColumns = [
                            ...
                            {caption: 'תוכן', fieldName: 'Description', type: 'string', width: '400px', color: '#303033'},
                           ];

      GridHandle.GridReset();

      setGridData(
          <GridHandle.GridWidget 
                        data={data} 
                        title='ניהול סטטוסים' 
                        ...
                        width='560px' 
                        height='900px' 
                        onSaveFuncName={onGridSaveFuncName} />);
    }
  ...
  async function showDataGridListTypes()
  {
    if (!isShowGrid)
    {
                    ...
                    <ul className="submenu">
                        ...
                        <li>
                            ...
                            <ul className="submenu">
                                ...
                                <li><a key={23} href="#statuses"      onClick={(e) => showDataGridStatuses()}>טבלת סטטוסים</a></li>
                                <li><a key={23} href="#list_types"    onClick={(e) => showDataGridListTypes()}>טבלת סוגי-פריט</a></li>
                            </ul>
                        </li>
...
function ListData({data, selectedItem, onSelectedItem}) 
{
                              ...
                              default:
                                return [...data].sort((a, b) => 
                                                  (
                                                    ...
                                                    : "")
                                                  -
                                                    ((a.LastUpdateDate && a.LastUpdateDate !== "")
  ...
  function handleChangeSort(e, text)
  {
    ...
    if (text === 'date_update')
    {
      setIsWithArchive(!isWithArchive);
    }
